...
const DadosPage: React.FC = () => {
  ...
  const [diasParaAtingirQuantidade, setDiasParaAtingirQuantidade] =
    useState<number>(0);
  ...
  useEffect(() => {
    ...
    const calcularMedia = (
      produtosPorDia: { [data: string]: number },
      ...
      const totalProdutos = datasFiltradas.reduce(
        (total, data) => total + produtosPorDia[data],
        0
      );
      return totalProdutos / datasFiltradas.length;
    };

    const calcularPorcentagemAumento = (
      valorAntigo: number,
      valorNovo: number
    ) => {
    ...
    const calcularMediaProdutosPorMes = (produtosPorMes: {
      [mes: string]: number;
    }) => {
      const totalMeses = Object.keys(produtosPorMes).length;
      const totalProdutos = Object.values(produtosPorMes).reduce(
    ...
    const calcularListaProdutosPorMes = (produtosPorMes: {
      [mes: string]: number;
    }) => {
    ...
    const produtosComDatas: Produto[] = data.map(
      (item: { data: string; produto: string }) => ({
        data: item.data,
    ...
    const datasAntes = produtosComDatas
      .filter(
        (item) => item.data >= dataInicioAntes && item.data < dataReferencia
      )
      .map((item) => item.data);
    ...
    const datasDepois = produtosComDatas
      .filter(
        (item) => item.data >= dataReferencia && item.data <= dataFimComparacao
      )
      .map((item) => item.data);
    ...
    const aumentoTotal = calcularPorcentagemAumento(
      quantidadeAntes,
      quantidadeDepois
    );
    ...
    const listaProdutosPorMesArray =
      calcularListaProdutosPorMes(produtosPorMes);
  ...
  return (
    <div className={styles.all}>
      ...
      <div className={styles.container}>
        ...
        <h1>Métricas GPT:</h1>
        <p>
          Quantidade de Produtos antes do dia 03/06: {quantidadeAntes} em 25
          dias
        </p>
        <p>
          Quantidade de Produtos depois do dia 03/06: {quantidadeDepois} em 25
          dias
        </p>
        <p>
          Dias para atingir a quantidade de antes: {diasParaAtingirQuantidade}
        </p>
